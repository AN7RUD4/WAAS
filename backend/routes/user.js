...
// // Waste detection function (extracted for reuse)
...
//   return { hasWaste, predictions };
// }

// // Detect waste endpoint (unchanged)
...
//     const { hasWaste, predictions } = await detectWaste(req.file.path);
...
// // Report Public Waste endpoint (modified with AI check)
// userRouter.post('/report-waste', authenticateToken, upload.single('image'), async (req, res) => {
//   const client = await pool.connect();
//   try {
//     console.log('Received report-waste request:', req.body);
...
//     // Step 1: Detect waste in the image
//     const { hasWaste, predictions } = await detectWaste(req.file.path);
//     if (!hasWaste) {
//       fs.unlinkSync(req.file.path);
//       await client.query('ROLLBACK');
//       console.log('No waste detected in image');
...
//     // Step 2: If waste is detected, proceed with insertion
//     const imageUrl = req.file.path;
//     const [lat, long] = location.split(',').map(Number);
//     if (isNaN(lat) || isNaN(long)) {
//       throw new Error('Invalid location format. Expected: "lat,long"');
...
// // Collection Requests endpoint (unchanged)
// userRouter.get('/collection-requests', authenticateToken, async (req, res) => {
//   const client = await pool.connect();
//   try {
//     console.log('Fetching collection requests for user:', req.user.userid);
